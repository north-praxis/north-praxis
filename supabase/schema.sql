-- North Praxis schema. Run in Supabase SQL Editor (make sure the editor
-- is NOT in read-only mode). Safe to re-run: creates use IF NOT EXISTS,
...
-- Row level security: anon may read active pages only. All writes go
-- through the service-role key (admin UI, MCP, server actions), which
-- bypasses RLS.
alter table page_content enable row level security;
alter table contact_submissions enable row level security;

drop policy if exists "anon can read active pages" on page_content;
create policy "anon can read active pages"
  on page_content for select
  using (status = 'active');

-- No anon policies on contact_submissions: service-role only.

-- Seed initial pages. Content matches lib/defaults.ts; the DB is the
-- source of truth once seeded.
insert into page_content (slug, content, status, updated_by) values
(
  'home',
  '{
    "title": "North Praxis | Strategy for mission-driven work",
    "metaDescription": "North Praxis helps nonprofits, foundations, DAFs, and impact investors turn values into clear plans and working systems.",
    "sections": [
      {
        "variant": "hero",
        "headline": "Strategy that helps mission-driven work find its bearings",
        "subhead": "Organizational design for nonprofits. Investment theses and systems for foundations, DAFs, and impact investors.",
...
(
  ...
  '{
    ...
    "metaDescription": "Chelsea Miller founded North Praxis to help mission-driven organizations and funders do their best work.",
    "sections": [
      {
        "variant": "intro",
        "eyebrow": "About",
...
(
  ...
  '{
    ...
    "metaDescription": "Strategic planning and organizational design for nonprofits. Investment thesis and systems work for foundations, DAFs, and impact investors.",
    "sections": [
      {
        "variant": "intro",
        "eyebrow": "Services",
...
(
  ...
  '{
    ...
    "metaDescription": "Get in touch with North Praxis about strategic planning, organizational design, or investment thesis work.",
    "sections": [
      {
        "variant": "contact",
